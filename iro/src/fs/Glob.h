...
#include "../containers/LinkedPool.h"
    ...
    case DoubleStar:
      {
        ...
        while (!dir_stack.isEmpty())
        {
          ...
          {
          case ConstantEntry:
            // TODO(sushi) mark constant entries as merged so we can skip this 
            //             check 
            if (s == next_part->raw) 
            {
              if (!callback(full))
...
err: // TODO(sushi) maybe do some kinda error handling sometime idrk if i care 
     //             for that yet
done:
  return;
}
